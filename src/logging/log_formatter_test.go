...
func TestLogger_FormatWithReader(t *testing.T) {
	...
	logger := logrus.New()
	result, err := LogFormatter{}.Format(fields.WithReader(reader)(logrus.NewEntry(logger)))
	require.Nil(t, err)
...
func TestLogger_Indentation(t *testing.T) {
	logger := logrus.New()
	result, err := LogFormatter{}.Format(logrus.NewEntry(logger).WithField("info", "test"))
	require.Nil(t, err)
	require.Equal(t, "test\n", string(result))

	result, err = LogFormatter{}.Format(logrus.NewEntry(logger).WithField("info", "test").WithField("indentation", 4))
	require.Nil(t, err)
	require.Equal(t, "    test\n", string(result))
}
...
func TestLogger_EmptyMapInfo(t *testing.T) {
	logger := logrus.New()
	result, err := LogFormatter{}.Format(logrus.NewEntry(logger).WithField("info", make(map[string]interface{}, 0)))
	require.Nil(t, err)
	require.Equal(t, "\n", string(result))
}
...
func TestLogger_Map(t *testing.T) {
	logger := logrus.New()
	result, err := LogFormatter{}.Format(logrus.NewEntry(logger).WithField("info", map[string]interface{}{
		"key1": "value1",
		"key2": "value2",
	}))
	require.Nil(t, err)
...
func TestLogger_MessageAndInfo(t *testing.T) {
	logger := logrus.New()
	log, err := LogFormatter{}.Format(logrus.NewEntry(logger).
...
func TestLogger_Colors(t *testing.T) {
	...
	log, _ := LogFormatter{}.Format(entry)
	// red
...
func TestLogger_LongPrefix(t *testing.T) {
	...
	entry.Level = logrus.DebugLevel
	log, err := LogFormatter{}.Format(entry)
	require.Nil(t, err)
